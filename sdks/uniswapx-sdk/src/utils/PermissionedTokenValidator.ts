...
import {
  DSTokenInterface,
  DSTokenInterface__factory,
  ISuperstateTokenV4__factory,
  Proxy__factory,
} from "../contracts";
...
export class PermissionedTokenValidator {
  ...
  static async preTransferCheck(
    ...
    if (token.interface === PermissionedTokenInterface.DSTokenInterface) {
      const tokenContract: DSTokenInterface = DSTokenInterface__factory.connect(
        resolvedTokenAddress,
        provider,
      );
      // eslint-disable-next-line @typescript-eslint/no-unused-vars
      const [code, _reason] = await tokenContract.preTransferCheck(
        from,
        to,
        value,
      );
      ...
      token.interface === PermissionedTokenInterface.ISuperstateTokenV4
    ) {
      const tokenContract = ISuperstateTokenV4__factory.connect(
        resolvedTokenAddress,
        provider,
      );
